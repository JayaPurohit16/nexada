  ...
  if (messenger != 0) {
    ...
    $.ajax({
      ...
      success: (data) => {
        ...
        messageInput.focus();
        // update info in view
        $(".messenger-infoView .info-name").text(data.fetch.first_name +' '+ data.fetch.second_name);
        $(".m-header-messaging .user-name").text(data.fetch.first_name +' '+ data.fetch.second_name);
        // Star status
        data.favorite > 0
