            ...
            <form action="{{ route('admin.skill.update') }}" method="POST" id="skillForm" enctype="multipart/form-data">
                ...
                <div class="row gy-4">
                    <div class="col-lg-4 col-md-6">
                        <label for="name" class="form-label fw-semibold text-primary-light text-sm mb-8">Name <span
                                class="text-danger-600">*</span></label>
                        <input type="text" class="form-control radius-8 @error('name') is-invalid @enderror" name="name"
                            id="name" placeholder="Name" maxlength="100" value="{{ $skill->name ?? '' }}">
                        @error('name')
                    ...
                    <div class="col-lg-4 col-md-6">
                        <label for="instrument_id"
                            class="form-label fw-semibold text-primary-light text-sm mb-8">Instrument <span
                                class="text-danger-600">*</span></label>
                        {{-- <input type="text"
                            class="form-control radius-8 @error('instrument_id') is-invalid @enderror"
                            name="instrument_id" id="instrument_id" placeholder="Instrument" maxlength="100"> --}}

                        <select name="instrument_id" id="instrument_id" class="form-control">
                            @if(isset($instruments))
                                <option value="">Select Instrument</option>
                                @foreach ($instruments as $instrument)
                                    <option value="{{ $instrument->id }}" @if($instrument->id == $skill->instrument_id) selected
                                    @endif>{{ $instrument->name }}</option>
                                @endforeach
                            @endif
                        </select>
                        @error('instrument_id')
                    ...
                    <div class="col-lg-4 col-md-6">
                        <label for="category" class="form-label fw-semibold text-primary-light text-sm mb-8">Category
                            <span class="text-danger-600">*</span></label>
                        <input type="text" class="form-control radius-8 @error('category') is-invalid @enderror"
                            name="category" id="category" placeholder="Category" maxlength="100"
                    ...
                    <div class="col-md-12">
                        ...
                        <textarea type="text" class="form-control radius-8 @error('description') is-invalid @enderror"
                            ...
                            maxlength="1000">{{ $skill->description ?? '' }}</textarea>
                        @error('description')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-semibold text-primary-light text-sm mb-8">External Videos
                            URL</label>
                        <div class="d-flex gap-2">
                            <div id="external-videos-container" class="flex-grow-1">
                                @foreach ($externalVideos as $externalVideo)
                                    <div class="input-group mb-3">
                                        <input type="text" class="form-control radius-8" name="external_videos[]"
                                            placeholder="External Video URL" maxlength="100"
                                            value="{{ old('external_videos[]', $externalVideo->name) }}">
                                        <button type="button" class="btn btn-primary border border-primary-600 remove-btn">Remove</button>
                                    </div>
                                @endforeach
                            </div>
                            <div id="add-external-video">
                                <button type="button" class="btn btn-primary border border-primary-600">Add</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-2">
                            <div id="notes-container" class="flex-grow-1">
                                @foreach ($skillNotes as $skillNote)
                                    <div class="input-group mb-3">
                                        <input type="text" class="form-control radius-8" name="notes[]" placeholder="Note"
                                            maxlength="500" value="{{ old('notes[]', $skillNote->name) }}">
                                        <button type="button"
                                            class="btn btn-primary border border-primary-600">Remove</button>
                                    </div>
                                @endforeach
                            </div>
                            <div id="add-note">
                    ...
                    <div class="col-md-6">
                        <label class="form-label fw-semibold text-primary-light text-sm mb-8">Supporting
                            Documents</label>
                        <div class="d-flex gap-2">
                            <div id="supporting-docs-container" class="flex-grow-1">
                                <div class="input-group mb-3">
                                    <input type="file" class="form-control radius-8" name="supporting_docs[]" multiple>
                                    {{-- <button type="button"
                                        class="btn btn-primary border border-primary-600 remove-btn">Remove</button>
                                    --}}
                                </div>
                            </div>
                            <div id="add-supporting-doc">
                                <button type="button" class="btn btn-primary border border-primary-600">Add</button>
                            </div>
                        </div>
                    </div>
                        ...
                        @foreach ($supportingDocs as $supportingDoc)
                            <div class="file-preview-container">
                                <a href="{{ asset($supportingDoc->name) }}" target="_blank">
                                    <img src="{{ asset('assets/images/file-icon.jpg') }}" class="img-thumbnail"
                                        style="width:200px;" alt="">
                                </a>
                                <iconify-icon icon="arcticons:trashcan"
                                    class="btn btn-outline-danger delete-button delete-file-btn"
                                    data-file-id="{{ $supportingDoc->id }}"></iconify-icon>
                            </div>
                        @endforeach
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="d-flex gap-2">
                            <div id="tutorial-videos-container" class="flex-grow-1">
                                <div class="input-group mb-3">
                    ...
                    <div class="col-md-6">
                        @foreach ($tutorialVideos as $tutorialVideo)
                            <div class="file-preview-container">
                                ...
                                <iconify-icon icon="arcticons:trashcan"
                                    class="btn btn-outline-danger delete-button delete-video-btn"
                                    data-video-id="{{ $tutorialVideo->id }}"></iconify-icon>
                            </div>
                        @endforeach
                    </div>
